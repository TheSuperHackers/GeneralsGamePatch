...
| C |   | Fixed issue that made it difficult to drive over the China Infantry Minigunner                                        |
...
| C |   | Fixed issue that delayed Demo GLA Technical suicide by up to 0.75 seconds                                             |
...
| C |   | Fixed non-functional suicide button on Demo GLA Demo Combat Cycle                                                     |
|   |   | Fixed missing Radar icon on China Command Center                                                                      |
|   |   | Added Supply Lines Upgrade icon to USA Chinooks                                                                       |
...
|   |   | Added missing unit drop to Reinforcement Pad for Boss General                                                         |
|   |   | Fixed wrong laser effect and sound of USA Laser Turret when attacking air targets                                     |
|   |   | Fixed inconsistent geometry sizes of Outpost models from China factions                                               |
|   |   | Fixed wrong Stop button icon location of Boss Patriot Battery                                                         |
|   |   | Fixed wrong tooltip of Boss Nationalism Upgrade after Mines upgrade                                                   |
|   |   | Added missing Neutron Mines to Boss SCUD Storm                                                                        |
|   |   | Added missing Chemical Suits upgrade icon to Boss Infantry units                                                      |
|   |   | Fixed issue where China Nuke Tactical Nuke Mig Upgrade could not be built on low power                                |
|   |   | Added missing Stop button to GLA Tunnel and GLA Palace                                                                |
|   |   | Fixed wrong button placements of USA Firebase                                                                         |
|   |   | Added missing Scrap Salvage to Boss Tomahawk Launcher                                                                 |
|   |   | Added missing Scrap Salvage to USA05 Campaign SCUD Launcher                                                           |
